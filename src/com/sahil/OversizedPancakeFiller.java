...
public class OversizedPancakeFiller {
    public int magic(String str, int flip) {
    ...
    public int doable(StringBuilder str, int flip) {
        ...
        for(int j = 0; j < flip; j++) {
            str.setCharAt(j+index, str.charAt(j+index) == '+'? '-' : '+');
        }
        return doable(new StringBuilder(str.substring(index+1)), flip) + 1;
    }
}
